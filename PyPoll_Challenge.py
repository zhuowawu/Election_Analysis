...
with open(file_to_save, "w") as txt_file:
    ...
    for county_names in county_votes:
        ...
        votes2 = county_votes[county_names]

        # 6c: Calculate the percentage of votes for the county.
        vote_percentage2 = float(votes2) / float(total_votes)* 100

        # 6d: Print the county results to the terminal.
        print (f"{county_names}: {vote_percentage2:.1f}% ({votes2:,})\n")

        # 6e: Save the county votes to a text file.
        txt_file.write(f"{county_names}: {vote_percentage2:.1f}% ({votes2:,})\n")

        # 6f: Write an if statement to determine the winning county and get its vote count.
        if (votes2 > winning_votes) and (vote_percentage2 > winning_percentage):
            winning_votes = votes2
